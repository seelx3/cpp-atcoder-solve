...
int main() {
  ...
  auto dp = make_vec(N + 1, W, 0LL);
  auto dpr = make_vec(N + 1, W, 0LL);
  ...
  REPR(i, N) {
    REP(j, W) {
      if (j - weight[i] >= 0)
        chmax(dpr[i][j], dpr[i + 1][j - weight[i]] + value[i]);
      chmax(dpr[i][j], dpr[i + 1][j]);
    }
  }
  ...
  REP(i, N) {
    REP(j, W) { // j: 0 ~ W-1
      chmax(ans, value[i] + dp[i][j] + dpr[i + 1][W - 1 - j]);
    }
  }
