...
int main() {
  ...
  REP(i, N) {
    edges.emplace_back(edge{i, SKY, X[i]});
    edges.emplace_back(edge{i, SEA, Y[i]});
  }
  REP(i, M) {
    int a = (int)input() - 1;
    int b = (int)input() - 1;
    ll z = input();
    edges.emplace_back(edge{a, b, z});
  }
  ...
  ll ans = INF;
  REP(type, 4) { // 1st bit: sky, 2nd bit: sea
    ll sum = 0;
    dsu uf(N + 2);
    for (auto [u, v, cost] : edges) {
      if (v == SKY && !(type & 1)) continue;
      if (v == SEA && !(type & 2)) continue;
      if (uf.same(u, v)) continue;
      sum += cost;
      uf.merge(u, v);
    }
    bool ok = true;
    REP(i, N - 1) if (uf.leader(i) != uf.leader(i + 1)) ok = false;
    if (ok) chmin(ans, sum);
  }
