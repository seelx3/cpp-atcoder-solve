...
int main() {
  ...
  vector<ll> hh, ww;
  REP(i, H) {
    if (cnt_h[i] == mx_h) { hh.push_back(i); }
  }
  REP(i, W) {
    if (cnt_w[i] == mx_w) { ww.push_back(i); }
  }
  ...
  ll ans = max(0LL, mx_h + mx_w - 1);

  bool ok = false;
  REP(i, SZ(hh)) {
    REP(j, SZ(ww)) {
      if (st.count({hh[i], ww[j]}) == 0) {
        chmax(ans, mx_h + mx_w);
        ok = true;
        break;
      }
  ...
  REP(i, M) { chmax(ans, cnt_h[h[i]] + cnt_w[w[i]] - 1); }
