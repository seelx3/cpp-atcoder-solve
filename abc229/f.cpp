...
int main() {
  ...
  // dp[i][j][k] : iまで見たときの同じ色の頂点間の辺の重みの和
  // jは頂点iの色, kは頂点1の色
  auto dp = make_vec(N + 1, 2, 2, INF);
  ...
  REPE(i, 1, N) {
    ...
    REP(j, 2) REP(k, 2) {
      // 頂点iが頂点i-1と同じ色
      chmin(dp[i][j][k], dp[i - 1][j][k] + B[i - 2] + (j == 0 ? A[i - 1] : 0));

      // 頂点iが頂点i-1と違う色
      chmin(dp[i][j][k], dp[i - 1][j ^ 1][k] + (j == 0 ? A[i - 1] : 0));
    }
  }
