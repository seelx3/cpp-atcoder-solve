...
int main() {
  ...
  auto dp = make_vec(N + 1, M + 2, M + 2, M + 2, (mint)0);

  dp[0][M + 1][M + 1][M + 1] = 1;

  REP(i, N) REP(j1, 1, M + 2) REP(j2, 1, M + 2) REP(j3, M + 2) {
    REP(k, 1, M + 1) {
      if (k <= j1) dp[i + 1][k][j2][j3] += dp[i][j1][j2][j3];
      else if (k <= j2) dp[i + 1][j1][k][j3] += dp[i][j1][j2][j3];
      else if (k <= j3) dp[i + 1][j1][j2][k] += dp[i][j1][j2][j3];
    }
  }

  mint ans = 0;
  REP(j1, 1, M + 1) REP(j2, j1 + 1, M + 1) REP(j3, j2 + 1, M + 1) {
    ans += dp[N][j1][j2][j3];
  }
