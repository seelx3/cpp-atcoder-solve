...
int main() {
  ...
  ll manhattan = 0;
  REP(i, N) manhattan += abs(x[i]);

  ll euclidean = 0;
  REP(i, N) euclidean += x[i] * x[i];
  double euclidean_sqrt = sqrt(euclidean);

  ll chebyshev = 0;
  REP(i, N) chmax(chebyshev, abs(x[i]));

  cout << manhattan << endl;
  cout << fixed << setprecision(15) << euclidean_sqrt << endl;
  cout << chebyshev << endl;
}
