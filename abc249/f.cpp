...
int main() {
  ...
  vector<pair<ll, ll>> ty(N);
  REP(i, N) cin >> ty[i].first >> ty[i].second;
  reverse(ALL(ty));
  ty.emplace_back(1, 0);
  ...
  REP(i, N + 1) {
    ...
    deb(i, t, y);
    if (t == 1) { // x = y_i
      if (ig_cnt < 0) continue;
      chmax(ans, y + sum);

      ig_cnt--;
      while (ig_cnt >= 0 && SZ(ms) > ig_cnt) {
        sum += *ms.rbegin();
      ...
      if (y >= 0) {
        ...
        if (SZ(ms) < ig_cnt) ms.insert(y);
        else {
          if (ig_cnt > 0 && y < *ms.rbegin()) {
            sum += *ms.rbegin();
            ms.erase(ms.find(*ms.rbegin()));
            ms.insert(y);
          } else {
