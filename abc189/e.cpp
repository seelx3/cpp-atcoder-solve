#ifndef INCLUDED_MAIN
#define INCLUDED_MAIN

#include <bits/stdc++.h>
using namespace std;

#include <atcoder/all>
using namespace atcoder;
using ll = long long;
#define ALL(v) v.begin(), v.end()
#define SZ(v) (int)(v.size())

#include __FILE__

using P = pair<ll, ll>;

int main() {
  int N = input();

  ll x, y;
  cin >> x >> y;
  vector<ll> dx(N), dy(N);
  REP(i, 1, N) {
    dx[i] = (ll)input() - x;
    dy[i] = (ll)input() - y;
  }

  // operation
  auto f1 = [](ll x, ll y) -> P { return {y, -x}; };
  auto f2 = [](ll x, ll y) -> P { return {-y, x}; };
  auto f3 = [](ll x, ll y, ll p) -> P { return {-x + 2 * p, y}; };
  auto f4 = [](ll x, ll y, ll p) -> P { return {x, -y + 2 * p}; };

  int M = input();
  vector<int> t(M), p(M, -1);
  REP(i, M) {
    cin >> t[i];
    if (t[i] >= 3) cin >> p[i];
  }

  // クエリ先読み
  int Q = input();
  vector<pair<pair<int, int>, int>> qs(Q);
  REP(i, Q) {
    qs[i].second = i;
    qs[i].first.first = (int)input();
    qs[i].first.second = (int)input() - 1;
  }

  sort(ALL(qs));

  // クエリ先読み計算
  int rot = 0;
  bool xflip = false;
  bool yflip = false;

  vector<P> ans(Q);

  int q_id = 0;
  while (qs[q_id].first.first == 0) {
    ans[qs[q_id].second] = {x + dx[qs[q_id].first.second],
                            y + dy[qs[q_id].first.second]};
    q_id++;
  }

  REP(i, M) {
    // operation
    if (t[i] == 1) {
      auto [nx, ny] = f1(x, y);
      x = nx;
      y = ny;
      if ((xflip && yflip) || (!xflip && !yflip)) rot = (rot + 1) % 4;
      else rot = (rot + 4 - 1) % 4;
    } else if (t[i] == 2) {
      auto [nx, ny] = f2(x, y);
      x = nx;
      y = ny;
      if ((xflip && yflip) || (!xflip && !yflip)) rot = (rot + 4 - 1) % 4;
      else rot = (rot + 1) % 4;
    } else if (t[i] == 3) {
      auto [nx, ny] = f3(x, y, p[i]);
      x = nx;
      y = ny;
      xflip = !xflip;
    } else if (t[i] == 4) {
      auto [nx, ny] = f4(x, y, p[i]);
      x = nx;
      y = ny;
      yflip = !yflip;
    }

    deb(x, y);
    while (q_id < Q && qs[q_id].first.first == i + 1) {
      int ansx = dx[qs[q_id].first.second];
      int ansy = dy[qs[q_id].first.second];

      if (rot == 1) {
        auto [nx, ny] = f1(ansx, ansy);
        ansx = nx;
        ansy = ny;
      } else if (rot == 2) {
        ansx = -ansx;
        ansy = -ansy;
      } else if (rot == 3) {
        auto [nx, ny] = f2(ansx, ansy);
        ansx = nx;
        ansy = ny;
      }

      if (xflip) ansx = -ansx;
      if (yflip) ansy = -ansy;

      ans[qs[q_id].second] = {ansx + x, ansy + y};

      q_id++;
    }
  }

  for (auto [xx, yy] : ans) {
    cout << xx << " " << yy << endl;
  }
}

/*-----------------------------------------------------------
-----------------------------------------------------------*/

#else // INCLUDED_MAIN

#define OVERLOAD_REP(_1, _2, _3, _4, name, ...) name
#define REP1(i, n) REP3(i, 0, n, 1)
#define REP2(i, a, b) REP3(i, a, b, 1)
#define REP3(i, a, b, c) for (int i = int(a); i < int(b); i += c)
#define REP(...) OVERLOAD_REP(__VA_ARGS__, REP3, REP2, REP1, )(__VA_ARGS__)

#define OVERLOAD_REPR(_1, _2, _3, _4, name, ...) name
#define REPR1(i, n) REPR3(i, 0, n, 1)
#define REPR2(i, a, b) REPR3(i, a, b, 1)
#define REPR3(i, a, b, c) for (int i = int(b) - 1; i >= int(a); i -= c)
#define REPR(...) OVERLOAD_REPR(__VA_ARGS__, REPR3, REPR2, REPR1, )(__VA_ARGS__)

// clang-format off
template<class T>bool chmax(T &a, const T &b) { if (a<b) { a=b; return 1; } return 0; }
template<class T>bool chmin(T &a, const T &b) { if (b<a) { a=b; return 1; } return 0; }

template< typename T, typename U >
static inline vector<U> make_vec(T&& n, U&& val) noexcept {
  return vector<U>(forward<T>(n), forward<U>(val)); 
}
template< typename T, typename... Args >
static inline decltype(auto) make_vec(T&& n, Args&&... args) noexcept {
  return vector<decltype(make_vec(forward<Args>(args)...))>(forward<T>(n), make_vec(forward<Args>(args)...));
}

struct input { 
  int n; input() {} input(int n_) : n(n_){};
  template <class T> operator T() { T ret; cin >> ret; return ret; }
  template <class T> operator vector<T>() { vector<T> ret(n); for (int i = 0; i < n; i++) cin >> ret[i]; return ret; }
};
// clang-format on

#ifdef LOCAL
#include <debug.hpp>
#define deb(...) debug::multi_print(#__VA_ARGS__, __VA_ARGS__)
#else
#define deb(...) (static_cast<void>(0))
#endif

#endif // INCLUDED_MAIN