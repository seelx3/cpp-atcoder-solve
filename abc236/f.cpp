...
int main() {
  int N = input();
  vector<ll> a = input((1 << N) - 1);
  vector<ll> idx((1LL << N) - 1);
  iota(ALL(idx), 0);
  sort(ALL(idx), [&](int x, int y) { return a[x] < a[y]; });

  // スパイスを{0, 1}の元を成分とするベクトルとして考える

  // スパイスを値段が小さい方から見ていき
  // それまでに選んだスパイスと独立ならば購入する

  // can_add[i] = 0 ならば辛さi はそれまでに選んだスパイスから作ることができない
  vector<char> can_add((1 << N));
  can_add[0] = 1; // 便宜上 1 で初期化

  auto add = [&](ll x) -> void {
    REP(i, (1 << N)) { can_add[i ^ x] |= can_add[i]; }
  };
