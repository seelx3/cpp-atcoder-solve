...
int main() {
  ...
  vector<ll> dp_l(SZ(col), INF), dp_r(SZ(col), INF);
  ...
  REP(i, 1, SZ(le)) {
    ll d = abs(ri[i] - le[i]);
    chmin(dp_l[i], dp_l[i - 1] + abs(ri[i] - le[i - 1]) + d);
    chmin(dp_l[i], dp_r[i - 1] + abs(ri[i] - ri[i - 1]) + d);
    chmin(dp_r[i], dp_l[i - 1] + abs(le[i] - le[i - 1]) + d);
    chmin(dp_r[i], dp_r[i - 1] + abs(le[i] - ri[i - 1]) + d);
  }

  deb(dp_l);
  deb(dp_r);

  cout << min(*dp_l.rbegin(), *dp_r.rbegin()) << '\n';
}
