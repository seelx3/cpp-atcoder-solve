...
ll myFloor(ll x, ll m = MAN) {
  return x > 0 ? x / m * m : (x - (m - 1)) / m * m;
}

ll myCeil(ll x, ll m = MAN) {
  return x > 0 ? (x + (m - 1)) / m * m : x / m * m;
}

int main() {
  double _X, _Y, _R;
  cin >> _X >> _Y >> _R;
  ll X = round(_X * MAN), Y = round(_Y * MAN), R = round(_R * MAN);
  ll ans = 0;

  ll sx = myCeil(X - R, MAN), ex = myFloor(X + R, MAN);

  REPE(i, sx, ex, MAN) {
    ll sy = myCeil(Y - mySqrt(R * R - (i - X) * (i - X)), MAN);
    ll ey = myFloor(Y + mySqrt(R * R - (i - X) * (i - X)), MAN);
