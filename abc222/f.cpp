...
int main() {
  ...
  // 木の直径を求める
  // 直径の両端のいずれかで、旅費が最大となる

  auto dist_from = [&](int s) {
  ...
  auto dist0 = dist_from(0);
  int e1 = max_element(ALL(dist0)) - dist0.begin();
  auto dist1 = dist_from(e1);
  int e2 = max_element(ALL(dist1)) - dist1.begin();
  auto dist2 = dist_from(e2);
