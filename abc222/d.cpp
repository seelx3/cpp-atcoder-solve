...
int main() {
  ...
  // dp[i][j] := i番目まで見てi個目がjとなるような組み合わせ
  // dp[i][j] = dp[i-1][j] + dp[i-1][j-1] + dp[i-1][j-2] + ... + dp[i-1][0]

  int MX = 3000;
  auto dp = make_vec(N + 1, MX + 1, (mint)0);

  vector<mint> sum(MX + 1, 1);
  REP(i, N) {
    REP(j, a[i], b[i] + 1) { dp[i + 1][j] = sum[j]; }
    REP(j, MX + 1) {
      if (j == 0) sum[j] = dp[i + 1][j];
      else sum[j] = sum[j - 1] + dp[i + 1][j];
    }
  }

  cout << sum[b[N - 1]].val() << endl;
}
