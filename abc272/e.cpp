...
int main() {
  ...
  // v[i]: Aにi回操作を行ったときのAの要素の値で0~Nの範囲に含まれる値の集合
  // 調和級数より、v[i]に含まれる要素数の和は N log N くらい
  vector<vector<int>> v(M + 1);

  REP(i, N) {
    int l = (A[i] >= 0 ? 0 : (abs(A[i]) + i) / (i + 1));

    for (int j = l; A[i] + (i + 1) * j <= N && j <= M; j++) {
  ...
  REPE(i, 1, M) {
    vector<bool> b(SZ(v[i]) + 1);
    REP(j, SZ(v[i])) if (v[i][j] <= SZ(v[i])) b[v[i][j]] = true;
    REP(j, SZ(v[i]) + 1) {
      if (!b[j]) {
        ans.push_back(j);
        break;
