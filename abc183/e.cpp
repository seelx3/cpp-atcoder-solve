...
int main() {
  ...
  auto dp = make_vec(H, W, (mint)0);
  auto sumr = make_vec(H, W, (mint)0);
  auto sumrd = make_vec(H, W, (mint)0);
  auto sumd = make_vec(H, W, (mint)0);
  dp[0][0] = 1;

  REP(i, H) REP(j, W) {
    if (S[i][j] == '#') continue;

    if (j) dp[i][j] += sumr[i][j - 1];
    if (i && j) dp[i][j] += sumrd[i - 1][j - 1];
    if (i) dp[i][j] += sumd[i - 1][j];

    if (j) sumr[i][j] = sumr[i][j - 1];
    if (i && j) sumrd[i][j] = sumrd[i - 1][j - 1];
    if (i) sumd[i][j] = sumd[i - 1][j];
