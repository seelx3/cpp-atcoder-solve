...
int main() {
  ...
  // 連結している頂点数が 1->2のとき、2->3のとき、...
  // のそれぞれについて、必要な移動回数の期待値を求める
  // i -> i+1 のときに必要な移動回数の期待値 =
  //   (まだ訪れていない頂点を訪れる確率) *
  //   (未訪問の頂点を訪れるまでに、既に訪れた頂点に移動する回数の期待値)
  // 式を考えてwolframalpha先輩に投げる

  double ans = 0;
  REPE(i, 1, N - 1) { ans += ((N - i) / N) * N * N / ((N - i) * (N - i)); }
