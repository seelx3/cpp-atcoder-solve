...
int main() {
  ...
  REP(i, n) REP(j, n) {
    if (i == j) continue;
    if (*s[i].rbegin() == *s[j].begin()) G[i].push_back(j);
  }

  // 状態数は高々n*(1<<n)
  vector<vector<int>> memo(n, vector<int>((1 << n), -1));

  // 頂点uを選択して状態Sであるときに、勝つことができるか
  function<bool(int, int)> dfs = [&](int u, int S) -> bool {
    bool win = true;
    // 次の番の人が勝つような手を持っていれば win = false
    for (auto v : G[u]) {
      if (S >> v & 1) continue;
      if (memo[v][S | (1 << v)] != -1) {
        if (memo[v][S | (1 << v)]) win = false;
      } else {
        if (memo[v][S | (1 << v)] = dfs(v, S | 1 << v)) win = false;
      }
    }
    return win;
  };
