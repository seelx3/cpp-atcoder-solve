...
template <class Tp>
auto has_begin(int)
    -> decltype(std::begin(std::declval<Tp>()), std::true_type{});
template <class Tp> auto has_begin(...) -> std::false_type;
template <class Tp>
...
[[maybe_unused]] constexpr bool is_iterable_container_v =
    decltype(has_begin<Tp>(int{}))::value;
template <class Tp>
...
auto check_elem(int) -> decltype(*std::begin(std::declval<Tp>()));
...
std::enable_if_t<is_iterable_container_v<Container>> out(const Container& arg) {
  if (std::distance(std::begin(arg), std::end(arg)) == 0) {
    os << "<empty container>";
    return;
  }
  os << "[ ";
  std::for_each(std::begin(arg), std::end(arg),
...
void multi_print(std::string_view names, const Tp& arg, const Ts&... args) {
  if constexpr (sizeof...(Ts) == 0) {
    names.remove_suffix(std::distance(
        names.rbegin(),
        std::find_if_not(names.rbegin(), names.rend(),
    ...
    const std::size_t first_varname_length =
        comma_pos -
        std::distance(
            names.rend() - comma_pos,
            std::find_if_not(names.rend() - comma_pos, names.rend(),
                             [](const char c) { return std::isspace(c); }));
    ...
    const std::size_t next_varname_begins_at = std::distance(
        names.begin(),
        std::find_if_not(names.begin() + comma_pos + 1, names.end(),
                         [](const char c) { return std::isspace(c); }));
